...
const portfolio = () => {
  ...
  const likeBtns = document.querySelectorAll('.portfolio__subject-like-btn');
  ...
  likeBtns.forEach(item => {
    item.addEventListener('click', async (e) => {
      ...
      if(!localData[`${itemName}`]) {
        ...
        const json = JSON.stringify({nameApp: `${itemName}`, likesCount: `${newLikeCount}`});
        ...
        if(success === 'ok') {
          ...
          localStorage.setItem('portfolioLikes', `${JSON.stringify(localData)}`);
        }
      }
    });
  });
